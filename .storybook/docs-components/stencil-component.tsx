...
const getTableHeader = (headers: string[]): string =>
  `| ${headers.join(' | ')} |`;
const getTableHeaderSeperator = (count: number): string =>
...
const angularExample = (metadata: JsonDocsComponent): string => {
  const inputs = metadata.props.map(
    p => `[${p.attr || p.name}]="${p.attr || p.name}"`,
  );
  const outputs = metadata.events.map(
    e =>
      `(${e.event})="on${uppercaseFirst(e.event)}(${
...
const reactExample = (metadata: JsonDocsComponent): string => {
  ...
  const outputs = metadata.events.map(
    e =>
      `on${uppercaseFirst(e.event)}={(${
        e.detail ? 'ev' : ''
      }) => handle${uppercaseFirst(e.event)}(${e.detail ? 'ev.detail' : ''})}`,
  );
...
const plainJsExample = (metadata: JsonDocsComponent): string => {
  const inputs = metadata.props.map(
    p => `el.setAttribute('${p.attr || p.name}', ${p.name});`,
  );
  const inputsAlt = metadata.props.map(
    p => `// el['${p.attr || p.name}'] = ${p.name};`,
  );
  const outputs = metadata.events.map(
    e =>
